...
This library is a blazingly fast multi-model AI chat server for use with OpenBB Workspace, powered by Snowflake.

It lets you chat with your data iteratively, and generate validated SQL queries using natural language.

## Installation

- Python environment: `>=3.10,<3.14`

Pre-compiled binaries for Linux, macOS, and Windows are found in the `./wheelhouse` folder. Select the appropriate wheel to meet the requirements of the environment.
...
I maintain a structured record of all messages in the current chat session — including:
...
This allows me to recall everything that’s been done without re-running queries or asking redundant questions.
...
- Context inspection: You can ask `/history` or `/current` to view what’s stored, including message counts, tool calls, and token usage.

You can manage context directly:

- `/history` → Lists all messages, tool calls, and operations in the conversation.
- `/current` → Shows current context settings, token usage, and model configuration.
- `/clear` → Resets context (starts fresh).
- `/help` → Lists all system-level commands.

Here’s how I use these capabilities intelligently:

- Preservation of tool outputs: I never summarize or paraphrase raw Snowflake data — those are always kept verbatim in context.
